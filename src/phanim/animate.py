...
class Transform(Animation):
    ...
    def update(self):
        self.object.transformFunction(
            self.t, self.oldPhobject, self.newPhobject)
        self.object.position = interp(
            self.oldPhobject.position, self.newPhobject.position, self.t)
...
class Move(Animation):
    ...
    def update(self):
        self.object.setPosition(
            interp(self.oldPhobject.position, self.target, self.t))
...
class Shift(Move):
    ...
    def update(self):
        self.object.setPosition(interp(
            self.oldPhobject.position, self.oldPhobject.position+self.target, self.t))
    ...
    def updateAndPrint(self):
        if self.playingAnimations[-1].currentFrame/self.playingAnimations[-1].duration > self.lagRatio:
            if len(self.playingAnimations) < len(self.animations):
                self.playingAnimations.append(
                    self.animations[len(self.playingAnimations)])
...
def makeGrid(n_horizontal=8, n_vertical=5):
    grids = DGrid(n_horizontal=n_horizontal,
                  n_vertical=n_vertical).groupObjects
    animations = []
    for grid in grids:
